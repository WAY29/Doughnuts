...
def change_prompt():
    ...
    if (namespace == "main"):
        ...
        print(color.yellow("[rs|reshell] ") + color.cyan("(Only for both system is linux) (Testing command) Bind a port and waiting for target connect back to get a pty shell. eg: reshell {lhost} {port} {fakename=/usr/lib/systemd}"))
